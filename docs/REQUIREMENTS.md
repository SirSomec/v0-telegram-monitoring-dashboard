...
| `SMTP_HOST`, `SMTP_PORT`, `SMTP_USER`, `SMTP_PASSWORD`, `SMTP_FROM`, `SMTP_USE_TLS` | Настройки SMTP для отправки писем (сброс пароля). Если не заданы — письмо не отправляется, ссылка логируется (для разработки). |
...
2. **Включить в навигацию:** пункты «Группы», «Уведомления» (и при необходимости «Оплата») после реализации соответствующих фич.
