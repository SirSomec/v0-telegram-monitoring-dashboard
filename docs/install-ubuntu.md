...
## 7b. Регистрация «ничего не делает» (кнопка жмётся, но нет ни перехода, ни сообщения)

Контейнеры работают, но после нажатия «Зарегистрироваться» ничего не происходит или ошибка не видна. Что проверить:

1. **Сообщение об ошибке на странице**  
   После обновления кода фронт показывает текст ошибки с бэкенда (например, «Email already registered» или «Пароль минимум 8 символов»). Если видите такое сообщение под формой — исправьте данные и попробуйте снова.

2. **Вкладка Network (F12 → Network)**  
   Откройте её, нажмите «Зарегистрироваться» и найдите запрос к **`/auth/register`** (или к полному URL с портом **:8000**). Посмотрите:
   - **URL** — должен быть `http://IP_СЕРВЕРА:8000/auth/register`, а не `:3000`. Если идёт на 3000 — задайте `NEXT_PUBLIC_API_URL` в `.env` и пересоберите фронт (раздел 7a).
   - **Статус** — 200 (успех), 400/422 (ошибка валидации или «Email already registered»), 5xx (ошибка сервера).
   - **Ответ (Response)** — при 400/422 там будет текст ошибки; фронт теперь выводит его под формой.
...
3. **Логи бэкенда**  
   ...
   ```
   В другом окне/вкладке снова нажмите «Зарегистрироваться». В логах должно появиться обращение к `POST /auth/register`. Если запрос не приходит — проблема до бэкенда (URL, CORS, сеть). Если приходит и в логах есть traceback — по нему можно понять ошибку (БД, конфиг и т.д.).

4. **Пароль не менее 8 символов**  
   Бэкенд требует минимум 8 символов; при меньшей длине вернётся 422, и сообщение должно отображаться под формой.
