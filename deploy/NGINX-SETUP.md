# Настройка Nginx для доступа по домену (и по IP)

Чтобы по **домену** или **IP** открывалась главная страница и весь сайт, на сервере нужен Nginx как обратный прокси.

**Простой вариант (любой домен):** используйте `deploy/nginx-domain.conf` — замените в нём `YOUR_DOMAIN` на свой домен и следуйте комментариям в файле. Весь трафик идёт на фронт (порт 3000), Next.js сам проксирует API. См. также раздел «Доступ по домену» в [README.md](../README.md).

Ниже — детальная настройка для **integration-wa.ru** (и по IP).
